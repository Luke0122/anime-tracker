...
async function refresh() {
  ...
  if (!state.view) {
    ...
    state.view = state.seasons.some((s) => s.key === cur) ? cur : (state.seasons[0] ? state.seasons[0].key : 'stats');
  }
  renderSidebar();
  render();
}
...
function renderStats(c) {
  ...
  const bars = Object.keys(bySeason).sort().reverse().map((k) => {
    const st = statsFor(bySeason[k]);
    return { key: k, label: seasonLabel(k), eps: st.eps, total: st.total };
  });
  const maxEps = Math.max(1, ...bars.map((b) => b.eps));
  const top = all.filter((a) => a.rating != null).sort((a, b) => b.rating - a.rating).slice(0, 5);
  const avgText = s.avg != null ? s.avg.toFixed(1) : '—';
  c.innerHTML = `
    <div class="view-head"><h1>统计</h1><span class="sub">共 ${all.length} 部追番记录</span></div>
    ...
    ${bars.length ? `
      <div class="stats-section-title">各季度累计集数</div>
      <div class="quarter-bars">
        ${bars.map((b) => `
          <div class="quarter-bar-row">
            <span class="q">${esc(b.label)}</span>
            <div class="bar-track"><div class="bar-fill" style="width:${Math.round((b.eps / maxEps) * 100)}%"></div></div>
            <span class="bar-val">${b.eps} 集 / ${b.total} 部</span>
          </div>`).join('')}
      </div>` : ''}
    ${top.length ? `
  ...
  c.innerHTML = `
    ...
    <div class="settings-panel">
      <div class="field">
        ...
        <div class="hint" style="margin-top:6px">默认：D:\\ANIME\\日本TV动画信息。应用会读取季度文件夹里的 bangumi JSON；缺失时解析其中的「新番信息.docx」。</div>
      </div>
      ...
      <div class="field">
        ...
        <div class="settings-box">
          ...
          每日自动备份 ${state.backupCount} 份（每天首次修改时生成，保留最近 30 天）<br/>
          可通过顶部菜单「导出 JSON 备份」手动备份，或直接复制上面的数据文件。
        </div>
      </div>
    </div>
  `;
...
function formHTML() {
  ...
  <div class="form-grid">
    ...
    <div class="form-field"><label>季度 *</label><select id="f-season">${seasonOptions(modal.seasonKey)}</select></div>
    <div class="form-field"><label>状态</label><select id="f-status">${statusOpts}</select></div>
    <div class="form-field"><label>当前集数</label><input id="f-episode" type="number" min="0" step="1" value="0" /></div>
...
function openAddModal(opts = {}) {
  ...
  const sel = $('#m-season');
  sel.innerHTML = allSeasonOptions().map(([k, label]) =>
...
function fillForm(v) {
  ...
  if (v.season && $('#f-season').querySelector(`option[value="${v.season}"]`)) $('#f-season').value = v.season;
  if (v.status) $('#f-status').value = v.status;
  $('#f-episode').value = v.episode != null ? v.episode : 0;
...
async function loadSeasonShows(key) {
  ...
  try {
    ...
    if (modal.seasonSource === 'none') {
      hintEl.textContent = '该季度暂无可用的新番数据（没有 bangumi JSON，也没有 docx）。可以切到「搜 Bangumi」或「手动填写」。';
    } else {
      hintEl.textContent = `数据来源：${modal.seasonSource === 'bangumi-json' ? 'Bangumi JSON' : modal.seasonSource === 'docx' ? '新番信息 docx' : 'yuc JSON'} · 共 ${modal.seasonItems.length} 部`;
    }
  } catch (e) {
...
async function submitForm() {
  ...
  const entry = {
    ...
    episode: Number($('#f-episode').value || 0),
...
async function doComplete(id) {
  try {
    const a = state.anime.find((x) => x.id === id);
    await call(api.updateAnime(id, { status: 'completed' }));
...
async function saveSettings() {
  ...
  try {
    await call(api.updateSettings({ animeInfoBaseDir: base }));
    toast('设置已保存', 'success');
    await refresh();
  } catch (e) { /* 已提示 */ }
}
